...
def init_v4_pool():
    ...
    # We're going to create a WBTC/UNI pool with 1 btc = 10000 uni as initial price
    amount_0 = 10000 * 10**18
    amount_1 = 1 * 10**8
    # At this step no token is sent to the pool: these amounts are just used to compute the price as a ratio

    trx_params = (
        codec
        .encode
        .chain()
        .v4_initialize_pool(uni_wbtc_pool_key, amount_0, amount_1)  # 1 btc = 10000 uni
    ...
    initial_tick = sqrt_price_x96_to_floor_tick(sqrt_price_x96)
    assert slot_0[0] == sqrt_price_x96
    assert slot_0[1] == initial_tick
...
def mint_v4_position():
    ...
    # The initial price is 1 btc = 10000 uni, ie 1 uni = 1e-4 btc (uni is currency_0 in the pool)
    # We want to open a position on a range close to [0.95e-4, 1.05e-4]
    tick_lower = price_0_to_closest_tick(0.95e-4, 18, 8, 50)
    assert tick_lower == -322900
    tick_upper = price_0_to_closest_tick(1.05e-4, 18, 8, 50)
    assert tick_upper == -321900

    # liquidity
    sqrt_price_x96 = 7922816251426433400832  # current sqrt_price_x96, see pool init
    sqrt_price_x96_a = tick_to_sqrt_price_x96(tick_lower)  # lower sqrt_price_x96
    sqrt_price_x96_b = tick_to_sqrt_price_x96(tick_upper)  # upper sqrt_price_x96

    # We don't want to add more than 0.5 btc and 4300 uni, so max liquidity would be:
    amount_0_max = 4300 * 10**18
    amount_1_max = int(0.5 * 10**8)
    max_liquidity = compute_liquidity(
        sqrt_price_x96,
        sqrt_price_x96_a,
        sqrt_price_x96_b,
        amount_0_max,
    ...
    trx_params = (
        ...
        mint_position(
            ...
            tick_lower=tick_lower,  # 1 btc ~ 10535.8044 uni
            tick_upper=tick_upper,  # 1 btc ~ 9533.2377 uni
            liquidity=liquidity,
    ...
    liquidity_events = v4_poolm_contract.events.ModifyLiquidity().process_receipt(receipt, errors=DISCARD)
    assert liquidity_events[0]['args']['tickLower'] == tick_lower
    assert liquidity_events[0]['args']['tickUpper'] == tick_upper
    assert liquidity_events[0]['args']['liquidityDelta'] == liquidity
    print(liquidity_events)
    print()
    transfer_events = uni_contract.events.Transfer().process_receipt(receipt, errors=DISCARD)
    for transfer_event in transfer_events:
        if transfer_event["args"]["to"] == v4_poolm_address:
            if transfer_event["address"] == uni_address:
                assert transfer_event["args"]["amount"] == 4251059261901286719037  # uni
            if transfer_event["address"] == wbtc_address:
                assert transfer_event["args"]["amount"] == 46367218  # wbtc
