...
assert (
    eth_wbtc_pool_id.hex().upper()
    == "54C72C46DF32F2CC455E84E41E191B26ED73A29452CDD3D82F511097AF9F427E"
)
...
v4_state_view_address = Web3.to_checksum_address(
...
v4_state_view_contract = w3.eth.contract(
    address=v4_state_view_address, abi=v4_state_view_abi
)
...
def swap_exact_in_single_eth_to_wbtc():
    ...
    trx_params = (
        ...
        .settle_all(
            eth_address, eth_amount
        )  # or: settle(eth_address, V4Constants.OPEN_DELTA.value, True).
        .build_v4_swap()
        .build_transaction(
            account.address, eth_amount, block_identifier=w3.eth.block_number
        )
    )
    ...
    assert (
        receipt["status"] == 1
    ), f'receipt["status"] is actually {receipt["status"]}'  # status == 1 => trx success
...
def swap_exact_out_single_eth_to_wbtc():
    ...
    trx_params = (
        ...
        .settle_all(
            eth_address, eth_amount_max
        )  # or: settle(eth_address, V4Constants.OPEN_DELTA.value, True).
        .build_v4_swap()
        .sweep(
            FunctionRecipient.SENDER, eth_address, Wei(0)
        )  # Otherwise ETH stays on the contract
        .build_transaction(
            account.address, eth_amount_max, block_identifier=w3.eth.block_number
        )
    )
...
def init_v4_pool():
    ...
    trx_params = (
        ...
        .build_transaction(
            account.address, Wei(100 * 10**18), block_identifier=w3.eth.block_number
        )
    )
...
def permit2_ur(token_address):
    ...
    permit2_allowance = codec.fetch_permit2_allowance(
        account.address, token_address, ur_address
    )
...
def approve_permit2_for_token(token_contract):
    ...
    allowance = token_contract.functions.allowance(
        account.address, permit2_address
    ).call()
    print(allowance)
...
def mint_v4_position():
    ...
    trx_params = (
        ...
        .v3_swap_exact_in_from_balance(
            FunctionRecipient.ROUTER, Wei(5000 * 10**18), weth_uni_v3_path
        )
        .transfer(
            FunctionRecipient.CUSTOM, uni_address, Wei(5000 * 10**18), posm_address
        )
        .permit2_transfer_from(
            FunctionRecipient.CUSTOM, wbtc_address, Wei(int(0.5 * 10**8)), posm_address
        )
        .v4_posm_call()
        .mint_position(
        ...
        .build_transaction(
            account.address, Wei(20 * 10**18), block_identifier=w3.eth.block_number
        )
    )
    ...
    assert (
        weth_contract.functions.balanceOf(account.address).call() == 0
    )  # 100 * 10**18
    ...
    print(
        f"{new_wbtc_balance=}",
        new_wbtc_balance / 10**8,
        "diff:",
        (wbtc_balance - new_wbtc_balance),
    )
...
def swap_exact_out_uni_to_eth_and_pay_portion():
    ...
    pay_portion_recipient = Web3.to_checksum_address(
        "0x350D7640755FD89DF55eebb88DdFfa91Ab805cb7"
    )
    ...
    trx_params = (
        ...
        .take_portion(
            eth_address, pay_portion_recipient, 1000
        )  # 1000/10000 = 0.1 = 10% => 0.1 ETH
    ...
    print(
        "New ETH balance:",
        new_eth_balance / 10**18,
        "Diff:",
        (new_eth_balance - eth_balance) / 10**18,
    )
