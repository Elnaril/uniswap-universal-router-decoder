...
ur_address = Web3.to_checksum_address("0xeC8B0F7Ffe3ae75d7FfAb09429e3675bb63503e4")
...
def buy_usdt():
    ...
    encoded_input = (
        ...
        .encode
        .chain()
        .wrap_eth(FunctionRecipient.ROUTER, amount_in, revert_on_fail=False)
        # can chain one of the 2 following v3 swap functions:
        .v3_swap_exact_in_from_balance(FunctionRecipient.SENDER, amount_out_min, v3_path, revert_on_fail=False)
        # .v3_swap_exact_in(FunctionRecipient.SENDER, amount_in, amount_out_min, v3_path, payer_is_sender=False, revert_on_fail=False)  # noqa
    ...
    assert usdt_balance == 300483230634005393391, f"USDT balance was actually: {usdt_balance}"
...
def sell_usdt_in_2_parts():
    amount_in_1 = 150 * 10**18
    amount_in_2 = 200 * 10 ** 18
    amount_out_min = 0
    ...
    permit_data, signable_message = codec.create_permit2_signable_message(
        usdt_address,
        # amount_in_1 + amount_in_2,  # max/infinite = 2**160 - 1
        2**160 - 1,
    ...
    encoded_input = (
        ...
        .encode
        .chain()
        .permit2_permit(permit_data, signed_message, revert_on_fail=False)
        .v3_swap_exact_in(FunctionRecipient.SENDER, amount_in_1, amount_out_min, v3_path, payer_is_sender=True, revert_on_fail=False)  # Should succeed  # noqa
        .v3_swap_exact_in(FunctionRecipient.SENDER, amount_in_2, amount_out_min, v3_path, payer_is_sender=True, revert_on_fail=False)  # Should fail but the transaction should succeed  # noqa
        .build(codec.get_default_deadline())
    )
    ...
    usdt_balance = usdt_contract.functions.balanceOf(account.address).call()
    assert usdt_balance == 150483230634005393391, f"USDT balance was actually: {usdt_balance}"

    wbnb_balance = wbnb_contract.functions.balanceOf(account.address).call()
    assert wbnb_balance == 498734917976368908, f"WBNB balance was actually: {wbnb_balance}"

    print(" => SELL USDT for WBNB: OK")
