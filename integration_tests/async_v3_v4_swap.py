...
async_w3 = AsyncWeb3(AsyncWeb3.AsyncHTTPProvider("http://127.0.0.1:8545"))
...
weth_contract = async_w3.eth.contract(address=weth_address, abi=weth_abi)

wbtc_address = AsyncWeb3.to_checksum_address("0x2260fac5e5542a773aa44fbcfedf7c193bc2c599")
...
wbtc_contract = async_w3.eth.contract(address=wbtc_address, abi=wbtc_abi)
...
usdc_contract = async_w3.eth.contract(address=usdc_address, abi=usdc_abi)

link_address = AsyncWeb3.to_checksum_address("0x514910771AF9Ca656af840dff83E8264EcF986CA")
...
link_contract = async_w3.eth.contract(address=link_address, abi=link_abi)
...
async def check_initialization():
    assert await async_w3.eth.chain_id == chain_id
    assert await async_w3.eth.block_number == initial_block_number
    assert await async_w3.eth.get_balance(account.address) == 10000 * 10**18
    assert await wbtc_contract.functions.balanceOf(account.address).call() == 0
    assert await weth_contract.functions.balanceOf(account.address).call() == 0
    assert await usdc_contract.functions.balanceOf(account.address).call() == 0
    assert await link_contract.functions.balanceOf(account.address).call() == 0
    print(" => Initialization: OK")


async def swap_exact_in_single_eth_to_wbtc():
    ...
    trx_params = await (
        ...
        build_v4_swap().
        build_transaction(account.address, eth_amount, block_identifier=block_number)
    )
    raw_transaction = async_w3.eth.account.sign_transaction(trx_params, account.key).raw_transaction
    trx_hash = await async_w3.eth.send_raw_transaction(raw_transaction)
    receipt = await async_w3.eth.wait_for_transaction_receipt(trx_hash)
    assert receipt["status"] == 1, f'receipt["status"] is actually {receipt["status"]}'  # status == 1 => trx success

    eth_balance = await async_w3.eth.get_balance(account.address)
...
async def permit2_ur(token_address):
    ...
    trx_params = await (
        ...
        build_transaction(account.address, block_identifier=block_number)
    )
...
async def approve_permit2_for_token(token_contract):
    ...
    transaction_count = await async_w3.eth.get_transaction_count(account.address)
    trx_params = await contract_function.build_transaction(
        {
    ...
    allowance = await token_contract.functions.allowance(account.address, permit2_address).call()
    print(allowance)
...
async def swap_exact_in_wbtc_v3_usdc_v4_link():
    ...
    trx_params = await (
        ...
        .build_transaction(
            account.address,
            block_identifier=block_number  # because test is on local Anvil fork
        )
    )
...
async def launch_integration_tests():
    ...
    await swap_exact_in_single_eth_to_wbtc()

    await permit2_ur(wbtc_address)
    await approve_permit2_for_token(wbtc_contract)

    await permit2_ur(usdc_address)
    await approve_permit2_for_token(usdc_contract)

    await swap_exact_in_wbtc_v3_usdc_v4_link()


def main():
    anvil_pid = launch_anvil()
    try:
        asyncio.run(launch_integration_tests())
