...
def check_initialization():
    ...
    assert (
        w3.eth.block_number == initial_block_number
    )  # Anvil starts at the exact fork block
...
def buy_usdc():
    ...
    assert (
        usdc_balance == 4492413591
    )  # Expected USDC balance after buying with 1 ETH at block 23491937
...
def sell_usdc_part_2():
    ...
    usdc_balance = usdc_contract.functions.balanceOf(account.address).call()
    assert (
        usdc_balance == 3492413591
    )  # Expected USDC balance after selling another 500 USDC
