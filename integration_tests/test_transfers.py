...
recipients = tuple(
    Account.from_key(
        "0x"
        + hashlib.sha256(f"uniswap_test_recipient_{i}_anvil_fork".encode()).hexdigest()
    )
    for i in range(1, 7)
)
...
def buy_and_transfer():
    ...
    encoded_input = (
        ...
        # usdc transfer
        .transfer(
            FunctionRecipient.SENDER, usdc_address, usdc_amount_per_recipient
        )  # transfer usdc to account
        .transfer(
            FunctionRecipient.CUSTOM,
            usdc_address,
            usdc_amount_per_recipient,
        ...
        .transfer(
            ...
            eth_address,
            eth_amount_per_recipient,
            recipients[5].address,
        )  # transfer eth to 6th recipient  # noqa
        .unwrap_weth(
            FunctionRecipient.SENDER, 0
        )  # unwrap and send back all remaining eth to account
        .build()
    )
    ...
    assert (
        receipt["status"] == 1
    ), f'receipt["status"] is actually {receipt["status"]}'  # trx status

    assert (
        usdc_contract.functions.balanceOf(account.address).call()
        == usdc_amount_per_recipient
    )
    for i in range(4):
        assert (
            usdc_contract.functions.balanceOf(recipients[i].address).call()
            == usdc_amount_per_recipient
        )
    ...
    for i in range(4, 6):
        ...
        assert (
            eth_balance == eth_amount_per_recipient
    ...
    assert (
        initial_eth_amount - value + unwrap_weth_amount - gas_price * gas_used
        == account_eth_balance
    ...
    for _ in range(8):
        for j in range(6):
            chain = chain.transfer(
                FunctionRecipient.CUSTOM, eth_address, value, recipients[j].address
            )
