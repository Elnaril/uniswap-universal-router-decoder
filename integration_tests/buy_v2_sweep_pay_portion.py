...
account_2 = Account.from_key(
    "0x75b69019be9c48a713678b028ee4dbafd81949126357b28ced1f616581ade9ac"
)
...
def buy_uni_with_fees():
    ...
    assert (
        account_uni_balance > 0
    ), f"Actual account uni balance is {account_uni_balance}"
    assert (
        account_2_uni_balance > 0
    ), f"Actual account_2 uni balance is {account_2_uni_balance}"

    # Validate 1% fee distribution (with small tolerance for rounding)
    expected_account_2_amount = int(total_uni * 0.01)
    assert (
        abs(account_2_uni_balance - expected_account_2_amount) < 1000
