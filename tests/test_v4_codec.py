...
def test_v4_position_manager_call():
    ...
    assert (
        repr(decoded_input["inputs"][0][1]["unlockData"]["params"][1][0]) == "<Function SETTLE(address,uint256,bool)>"
    )  # noqa
...
@pytest.mark.parametrize(
    ...
    ((Wei(123456789012345678), "<Function WRAP(uint256)>"), (Wei(0), "<Function WRAP(uint256)>")),
)
...
@pytest.mark.parametrize(
    "amount_input, expected_result_func",
    ((Wei(123456789012345678), "<Function UNWRAP(uint256)>"), (Wei(0), "<Function UNWRAP(uint256)>")),
)
...
def test_v4_take_pair(currency_0_input, currency_1_input, recipient_input, expected_result_func):
    encoded_input = (
        codec.encode.chain()
        .v4_posm_call()
        .take_pair(currency_0=currency_0_input, currency_1=currency_1_input, recipient=recipient_input)
        .build_v4_posm_call(deadline=codec.get_default_deadline())
        .build(deadline=1732612928)
    )
    result = codec.decode.function_input(input_data=encoded_input)
