...
def test_get_default_expiration(codec):
    assert (
        -1
        < codec.get_default_expiration() - codec.get_default_expiration(30 * 24 * 3600)
        < 1
    )
...
@pytest.mark.parametrize(
    ...
    (
        (
            Web3.to_checksum_address("0x1944922D86209F7e5F3c88F2B2b4A8e737BeA9b1"),
            Web3.to_checksum_address("0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984"),
            19881602,
            (1461501637330902918203684832716283019655932542975, 1718529375, 1),
        ),  # noqa
        (
            Web3.to_checksum_address("0x1944922D86209F7e5F3c88F2B2b4A8e737BeA9b1"),
...
def test_fetch_permit2_allowance(
    wallet, token, block_identifier, expected_result, codec_rpc
):
